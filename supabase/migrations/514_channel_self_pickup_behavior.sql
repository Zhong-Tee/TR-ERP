-- Make customer pickup a configurable channel behavior instead of a SHOPP-only rule.

ALTER TABLE public.channels
  ADD COLUMN IF NOT EXISTS is_self_pickup BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN public.channels.is_self_pickup IS
'true = customer collects the order; tracking is cleared and parcel scanning is skipped, while WMS/QC/product scans remain required';

-- Preserve the behavior already deployed for SHOPP.
UPDATE public.channels
SET is_self_pickup = true
WHERE upper(btrim(channel_code)) = 'SHOPP';
...
BEGIN
  IF EXISTS (
    ...
    WHERE upper(btrim(c.channel_code)) = upper(btrim(COALESCE(NEW.channel_code, '')))
      AND c.is_self_pickup = true
  ) THEN
...
BEGIN
  ...
  IF COALESCE(array_length(p_order_ids, 1), 0) > 0 THEN
    ...
    SELECT
      ...
      COALESCE(c.is_self_pickup, false) AS is_self_pickup,
      units.expected,
      scans.scanned
    INTO v_incomplete
    FROM public.or_orders o
    LEFT JOIN public.channels c
      ON upper(btrim(c.channel_code)) = upper(btrim(COALESCE(o.channel_code, '')))
    LEFT JOIN LATERAL (
    ...
    WHERE o.id = ANY(p_order_ids)
      ...
      AND (
        (
          NOT COALESCE(c.is_self_pickup, false)
    ...
    IF FOUND THEN
      IF v_incomplete.tracking_number IS NULL AND NOT v_incomplete.is_self_pickup THEN
...
'Finalize packing atomically; configured self-pickup channels skip tracking validation but still require all active product units to be scanned.';
