-- Auto-convert RM -> FG from Roll Material Calculator config
-- Scope:
--   - Trigger only on movement types: 'gr' and 'adjust'
--   - Apply only when NEW.product_id is configured as RM in roll_material_configs
--   - Keep roll_usage_logs / dashboard calc logic unchanged
...
DECLARE
  ...
  v_note TEXT;
BEGIN
  -- Convert only from GR and stock adjustment events.
  IF NEW.movement_type NOT IN ('gr', 'adjust') THEN
    RETURN NEW;
  END IF;

  -- Convert only if source product is RM and mapped to an FG with configured ratio.
  SELECT
    c.fg_product_id,
    c.sheets_per_roll,
    c.cost_per_sheet
  ...
  WHERE c.rm_product_id = NEW.product_id
  ...
  IF v_fg_qty > 0 THEN
    v_unit_cost := COALESCE(v_cfg.cost_per_sheet, fn_get_current_avg_cost(v_cfg.fg_product_id), 0);
...
DROP TRIGGER IF EXISTS trg_auto_convert_rm_to_fg_on_movement ON inv_stock_movements;
CREATE TRIGGER trg_auto_convert_rm_to_fg_on_movement
AFTER INSERT ON inv_stock_movements
FOR EACH ROW
EXECUTE FUNCTION fn_auto_convert_rm_to_fg_on_movement();
