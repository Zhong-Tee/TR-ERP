-- 196: Fix rpc_next_work_order_name to upsert seed with GREATEST()
-- Problem: if (prefix,date_part) row already exists, ON CONFLICT DO NOTHING keeps an old last_r_no,
-- causing the generator to repeat numbers (e.g., latest is R3 but sequence row still at 2 -> returns R3).
...
BEGIN
  ...
  -- IMPORTANT: if row already exists, bump it up to at least the seed (never down)
  INSERT INTO or_work_order_sequences(prefix, date_part, last_r_no)
  VALUES (v_prefix, v_date, v_seed)
  ON CONFLICT (prefix, date_part)
  DO UPDATE SET
    last_r_no = GREATEST(or_work_order_sequences.last_r_no, EXCLUDED.last_r_no),
    updated_at = now();
