-- Keep packing finalization aligned with the Packing UI after partial bill
-- cancellation. Cancelled item rows remain in or_order_items for audit, but
-- they must not contribute to the number of units required for shipment.
...
BEGIN
  ...
  IF COALESCE(array_length(p_order_ids, 1), 0) > 0 THEN
    ...
    LEFT JOIN LATERAL (
      -- Match normalizedLineQuantity() in the Packing UI.
    ...
    IF FOUND THEN
      IF v_incomplete.tracking_number IS NULL THEN
...
'Finalize packing atomically; shipment validation counts only active order items whose cancellation_stock_action is blank.';
