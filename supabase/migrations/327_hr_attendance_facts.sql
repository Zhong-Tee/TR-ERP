-- =============================================================================
-- hr_attendance_facts — ชั้น "ข้อเท็จจริง" ของคะแนนวินัย (ไม่มีคะแนนอยู่ในนี้)
--
-- คืนแถวละ 1 คน 1 วัน พร้อมทุกอย่างที่ engine ต้องใช้ตัดสิน:
--   ตารางเวลาที่ควรเข้า/ออก · เวลาจริง (หรือเวลาที่หัวหน้ารับรอง) · ใบลา · OT
-- เวลาทั้งหมดคืนเป็น "นาทีนับจากเที่ยงคืนของ work_date ตามเวลาไทย" เพื่อให้
-- ฝั่ง TS คำนวณได้แบบ deterministic โดยไม่ต้องพึ่ง timezone ของเครื่อง
--
-- ลำดับการตัดสินประเภทวัน: ปฏิทินรายคน > วันหยุดบริษัท > วันทำงานประจำสัปดาห์
-- ลำดับตารางเวลา: override รายวัน > ตารางของพนักงาน > ตารางค่าเริ่มต้น > hr_clock_settings
--
-- ข้อจำกัดที่รู้ตัว: กดออกงานหลังเที่ยงคืน hr_time_entries.work_date จะเป็นวันถัดไป
-- ทำให้วันนั้นขึ้นว่า "ไม่มีเวลาออก" — เคสนี้ให้หัวหน้ารับรองเวลาแทน
--
