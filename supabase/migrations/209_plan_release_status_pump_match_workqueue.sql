-- 209: ย้ายบิลจากจัดการใบงาน → ใบสั่งงาน — สถานะ PUMP ต้องสอดคล้องกับหน้าใบสั่งงาน
-- เดิม: ทุกช่องทางถูกตั้งเป็น 'ใบสั่งงาน' ทำให้บิล PUMP ไม่ตรงกับ WorkOrderSelectionList
--       (รองรับเฉพาะ status คอนเฟิร์มแล้ว / เสร็จสิ้น)
-- ใหม่: ช่องทาง PUMP → 'เสร็จสิ้น' (สอดคล้องกับ doCancelWorkOrder ใน WorkOrderManageList)
...
BEGIN
  ...
  SET
    ...
    status = CASE
      WHEN trim(both FROM coalesce(o.channel_code, '')) = 'PUMP' THEN 'เสร็จสิ้น'::text
      ELSE 'ใบสั่งงาน'::text
    END,
...
'ย้ายบิลกลับคิว Plan→ใบสั่งงาน: ช่องทางทั่วไป status=ใบสั่งงาน, PUMP status=เสร็จสิ้น (ให้ตรงกับหน้าเลือกบิลสร้างใบงาน)';
