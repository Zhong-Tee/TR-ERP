-- Marketplace shipping badges: configurable colors and a blue "ส่งวันนี้" default badge.
BEGIN;

ALTER TABLE mp_orders
  ADD COLUMN IF NOT EXISTS urgency_color TEXT;
...
BEGIN
  ...
  WITH matched AS (
    SELECT DISTINCT ON (o.id)
      ...
      CASE
        WHEN rule.item->>'color' IN ('blue', 'orange', 'green', 'purple', 'pink', 'slate')
          THEN rule.item->>'color'
        ELSE 'orange'
  ...
  SET shipping_option = matched.shipping_option,
      ...
      urgency_color = matched.urgency_color
  FROM matched
  ...
  SET shipping_option = source.shipping_option,
      ...
      urgency_color = source.urgency_color
