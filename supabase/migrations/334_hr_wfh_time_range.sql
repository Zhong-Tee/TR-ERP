-- =============================================================================
-- คำขอ WFH: เพิ่มช่วงเวลาทำงาน (start_time / end_time)
--
-- หลังอนุมัติ การลงเวลาเข้า-ออกของวันนั้นจะอ้างอิงช่วงเวลาที่ขอ WFH
-- แทนตารางเวลามาตรฐาน (สาย/ออกก่อนเวลา คิดจากช่วงนี้)
-- คำขอเก่าที่ยังไม่มีเวลา (NULL) = ใช้ตารางเวลาเดิมเหมือนก่อนหน้า
--
...
COMMENT ON COLUMN hr_wfh_requests.start_time IS 'เวลาเริ่มงานที่ขอ WFH (NULL = ใช้ตารางเวลามาตรฐาน)';
COMMENT ON COLUMN hr_wfh_requests.end_time IS 'เวลาเลิกงานที่ขอ WFH (NULL = ใช้ตารางเวลามาตรฐาน)';

-- กรอกเวลาต้องครบคู่ และเวลาเลิกต้องหลังเวลาเริ่ม
ALTER TABLE hr_wfh_requests DROP CONSTRAINT IF EXISTS hr_wfh_requests_time_range_check;
ALTER TABLE hr_wfh_requests ADD CONSTRAINT hr_wfh_requests_time_range_check
  CHECK (
...
-- =============================================================================
-- hr_attendance_facts: วัน WFH ที่อนุมัติแล้วและระบุช่วงเวลาไว้
-- → ใช้ช่วงเวลานั้นเป็น expected_start_min / expected_end_min
-- ลำดับความสำคัญ: คำขอ WFH > override รายวัน > ตารางของพนักงาน > ตารางค่าเริ่มต้น
-- (นอกนั้นเหมือน migration 330 ทุกประการ)
-- =============================================================================
...
BEGIN
  ...
  -- ต้องอยู่ก่อน eff เพราะ eff อ้างช่วงเวลาของคำขอ WFH
  LEFT JOIN LATERAL (
  ...
  CROSS JOIN LATERAL (
    SELECT
      COALESCE(wfh.start_time, cal.work_start, cal_sched.work_start, b.work_start) AS eff_start,
      COALESCE(wfh.end_time, cal.work_end, cal_sched.work_end, b.work_end) AS eff_end
  ) eff
  LEFT JOIN LATERAL (
