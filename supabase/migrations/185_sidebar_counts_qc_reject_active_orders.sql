-- Align get_sidebar_counts.qc_reject with fetchRejectItems() in src/lib/qcApi.ts:
-- count is_rejected rows only when the item is still tied to a non-cancelled order,
-- or when there is no or_order_items row for item_uid (same edge case as client).
-- Excluded statuses must stay in sync with FULFILLMENT_EXCLUDED_ORDER_STATUSES in src/lib/orderFlowFilter.ts
