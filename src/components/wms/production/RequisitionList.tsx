  ...
  return (
    <div className="p-4 space-y-4">
      <div className="bg-white border border-gray-200 shadow-sm p-4 rounded-2xl">
        <h2 className="text-xl font-black text-gray-900 mb-4">รายการใบเบิกของฉัน</h2>
        {requisitions.length === 0 ? (
          ...
          <div className="space-y-3">
            {requisitions.map((req) => {
              ...
              return (
                <div key={req.id} className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
                  <button
                    type="button"
                    onClick={() => toggleDetail(req.requisition_id)}
                    className="w-full text-left p-4 active:bg-gray-200 transition-colors"
                  >
                    <div className="flex items-start justify-between mb-2 gap-2">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className="text-xs font-black text-blue-600 break-words">{req.requisition_id}</span>
                      ...
                      <div className="text-xs text-gray-500 shrink-0 text-right">
                        ...
                        {req.notes && (
                          <div className="text-sm text-gray-600 font-medium break-words mt-1 max-w-[200px]">หมายเหตุ: {req.notes}</div>
                        )}
                      </div>
                    </div>
                      ...
                      {detailLoading ? (
                        ...
                        <div className="space-y-2">
                          ...
                          {detailItems.map((item, idx) => (
                            <div key={item.id} className="flex items-center gap-3 p-2.5 bg-white border border-gray-200 rounded-xl">
                              <div className="text-sm font-black text-gray-500 w-6 text-center shrink-0">{idx + 1}</div>
                              <img
                                src={imgUrl(item.product_code)}
                                className="w-14 h-14 object-cover rounded-lg shrink-0 border border-gray-300"
                                onError={(e) => {
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-bold text-gray-900 text-sm leading-tight">{item.product_name}</div>
                                <div className="text-[11px] text-gray-500 mt-0.5">รหัส: {item.product_code}</div>
                                {item.location && (
                                  <div className="text-[11px] text-red-500">จุดเก็บ: {item.location}</div>
                                )}
                              </div>
                              <div className="text-blue-700 font-bold text-base shrink-0 bg-blue-100 px-3 py-1 rounded-lg">
                                x{item.qty}
                              </div>
                            </div>
                          ))}
