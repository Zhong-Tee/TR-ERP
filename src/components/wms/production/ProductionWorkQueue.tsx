  ...
  return (
    <div className="p-3 space-y-3">
      ...
      {(!dept || dept === 'ALL') ? (
        ...
        <>
          {/* Department info */}
          <div className="rounded-lg bg-white border border-gray-200 shadow-sm px-3 py-2 flex flex-wrap items-center justify-between gap-2">
            <span className="font-semibold text-sm text-gray-900">แผนก: {dept}</span>
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{workflowLabel}</span>
          </div>

          {/* Queue by line */}
          {Array.from({ length: linesCount }, (_, lineIdx) => (
            <div key={lineIdx} className="rounded-xl border border-gray-200 bg-white shadow-sm p-2">
              {linesCount > 1 && (
                <h3 className="mb-2 border-b border-gray-200 pb-1.5 font-semibold text-sm text-gray-600">
                  Line {lineIdx + 1}
                </h3>
              )}
              <div className="flex flex-col gap-2">
                {lineJobs[lineIdx].length === 0 && (
                  <p className="text-center text-gray-400 text-xs py-4">ไม่มีงาน</p>
                )}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="font-bold text-base text-gray-900">{j.name}</div>
                          <div className="text-xs text-gray-500 mt-0.5">
                            ตัด: {fmtCutTime(j.cut)} | Qty: <b className="text-gray-700">{getEffectiveQty(j, dept, settings)}</b>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => setExpandedJob(isExpanded ? null : expKey)}
                          className="shrink-0 rounded-lg border border-gray-300 bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600 hover:bg-gray-200"
                        >
                      ...
                      {isAllDone ? (
                        <div className="mt-3 space-y-2">
                          <div className="rounded-xl border border-green-300 bg-green-100 py-2 text-center text-sm font-bold text-green-700">
                            ✓ เสร็จสมบูรณ์
                          </div>
                          <button
                            type="button"
                            onClick={() => backStep(j.id, dept)}
                            className="w-full rounded-lg border border-amber-300 bg-amber-50 py-2 text-sm font-medium text-amber-700 hover:bg-amber-100 active:bg-amber-200"
                          >
                            ↺ แก้ไข/ย้อนขั้นตอน
                          </button>
                        </div>
                        ...
                        <div className="mt-3 rounded-xl border border-gray-200 bg-gray-50 p-2.5">
                          ...
                          <div className="mt-2 flex justify-between border-t border-dashed border-gray-200 pt-2 text-[11px] text-gray-500">
                            <span>เวลาเริ่ม: <b className="text-blue-600">{startTime}</b></span>
                        ...
                        <div className="mt-3 space-y-1 border-t border-gray-200 pt-3">
                          {processNames.map((pName) => {
                            ...
                            return (
                              <div
                                key={pName}
                                className="flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-[11px]"
                              >
            ...
            <div className="px-5 pt-5 pb-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1.5 text-sm leading-relaxed text-gray-600">{modal.message}</p>
                </div>
              </div>
            </div>
            <div className={`flex gap-3 border-t border-gray-200 px-5 py-3.5 ${
