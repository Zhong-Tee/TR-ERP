...
const s = StyleSheet.create({
  ...
  infoLabel: { width: '17%', backgroundColor: '#e8eef7', padding: 8, fontWeight: 'bold' },
  infoValue: { width: '33%', padding: 8 },
  ...
  signature: { width: 90, height: 38, objectFit: 'contain', marginRight: 18 },
...
export default function PayrollSlipPDF({ company, item, monthLabel, paymentDate, ytd }: { company: HRCompany; item: PayrollItem; monthLabel: string; paymentDate: string; ytd: PayrollYtd }) {
  ...
  return <Document><Page size={[595.28, 841.89]} style={s.page} wrap={false}>
    ...
    <View style={s.info}><View style={s.infoRow}><Text style={s.infoLabel}>ชื่อ-สกุล</Text><Text style={s.infoValue}>{item.employee_name}</Text><Text style={s.infoLabel}>ตำแหน่ง</Text><Text style={s.infoValue}>{positionOnly}</Text></View><View style={s.infoRow}><Text style={s.infoLabel}>ประจำเดือน</Text><Text style={s.infoValue}>{monthLabel}</Text><Text style={s.infoLabel}>วันที่จ่าย</Text><Text style={s.infoValue}>{paymentDate || '-'}</Text></View></View>
