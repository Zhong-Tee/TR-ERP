...
export default function HRRequests() {
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    if (!q) return true
    const employee = item.employee ? `${item.employee.first_name} ${item.employee.last_name} ${item.employee.nickname || ''} ${item.employee.employee_code}` : ''
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-3 rounded-xl border bg-white p-4 shadow-sm">
      ...
      <select value={filter} onChange={(e) => setFilter(e.target.value)} className="rounded-lg border px-3 py-2 text-sm"><option value="">ทุกสถานะ</option>{Object.entries(HR_REQUEST_STATUS).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select>
    </div>
    <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
      {loading ? <p className="p-10 text-center text-slate-500">กำลังโหลด…</p> : filtered.length === 0 ? <p className="p-10 text-center text-slate-500">ไม่พบคำร้อง</p> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-slate-600"><tr><th className="p-3">วันที่</th><th className="p-3">ผู้ส่ง</th><th className="p-3">หัวข้อปัญหา</th><th className="p-3">เวลาที่เสีย</th><th className="p-3">สถานะ</th><th className="p-3 text-center">ดู</th></tr></thead><tbody className="divide-y">{filtered.map((item) => { const status = HR_REQUEST_STATUS[item.status]; return <tr key={item.id} className="hover:bg-slate-50"><td className="whitespace-nowrap p-3">{new Date(item.created_at).toLocaleDateString('th-TH')}</td><td className="p-3">{employeeName(item)}</td><td className="max-w-md p-3 font-medium">{item.problem_title}</td><td className="whitespace-nowrap p-3">{formatHRRequestDuration(item.time_lost)}</td><td className="p-3"><span className={`rounded-full px-2 py-1 text-xs font-bold ${status.color}`}>{status.label}</span></td><td className="p-3 text-center"><button onClick={() => open(item)} className="rounded-lg p-2 text-blue-600 hover:bg-blue-50"><FiEye /></button></td></tr>})}</tbody></table></div>}
    </div>
    {selected && <Modal open onClose={() => setSelected(null)} contentClassName="max-w-5xl">
      <div className="flex items-center justify-between border-b px-5 py-4"><h2 className="text-lg font-bold">รายละเอียดคำร้อง</h2><button onClick={() => setSelected(null)} className="text-2xl text-slate-400">×</button></div>
      <div className="space-y-4 p-5 text-sm">
        <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs text-slate-500">ผู้ส่ง</p><p className="font-semibold">{employeeName(selected)}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${HR_REQUEST_STATUS[selected.status].color}`}>{HR_REQUEST_STATUS[selected.status].label}</span></div>
