  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm"><span className="mb-1 block font-medium text-gray-700">ปี</span><input type="number" value={year} onChange={(e) => setYear(Number(e.target.value) || currentYear)} className={inputClass} /></label>
      ...
      <section>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200">
          <table className="w-full text-sm"><thead className="bg-gray-50"><tr><th className="px-3 py-2 text-left">ประเภทลา</th><th className="w-36 px-3 py-2 text-left">คงเหลือ (วัน)</th><th className="px-3 py-2 text-left">หมายเหตุ</th></tr></thead>
            <tbody>{leaveTypes.map((type) => <tr key={type.id} className="border-t border-gray-100"><td className="px-3 py-2 font-medium">{type.name}</td><td className="px-3 py-2"><input type="number" min={0} step="0.5" value={balances[type.id]?.days ?? '0'} onChange={(e) => setBalances((p) => ({ ...p, [type.id]: { ...p[type.id], days: e.target.value } }))} className={inputClass} /></td><td className="px-3 py-2"><input value={balances[type.id]?.note ?? ''} onChange={(e) => setBalances((p) => ({ ...p, [type.id]: { ...p[type.id], note: e.target.value } }))} className={inputClass} /></td></tr>)}</tbody>
          </table>
        </div>
      </section>
      <section>
