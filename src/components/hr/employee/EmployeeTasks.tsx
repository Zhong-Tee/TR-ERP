  ...
  return <>
        ...
        {shown.map((task) => {
          ...
          return <article key={task.id} className="bg-white border rounded-2xl overflow-hidden">
            ...
            {expanded && <div className="border-t p-4 space-y-4">
              ...
              <div><h3 className="font-medium text-sm mb-2">ประวัติเวลางาน</h3><div className="grid gap-3 rounded-xl bg-slate-50 p-3 text-xs"><div><div className="flex items-center justify-between gap-3"><span className="text-gray-500">รับทราบงาน</span><span className="font-medium text-gray-700">{taskTime(task.acknowledged_at)}</span></div><div className="mt-1 text-right text-blue-600">มอบหมาย → รับทราบ: {minutesBetween(task.created_at, task.acknowledged_at)}</div></div><div><div className="flex items-center justify-between gap-3"><span className="text-gray-500">เริ่มงาน</span><span className="font-medium text-gray-700">{taskTime(task.started_at)}</span></div><div className="mt-1 text-right text-emerald-600">เริ่มงาน → ส่งงาน: {minutesBetween(task.started_at, task.submitted_at)}</div></div><div><div className="flex items-center justify-between gap-3"><span className="text-gray-500">ส่งงาน</span><span className="font-medium text-gray-700">{taskTime(task.submitted_at)}</span></div><div className="mt-1 text-right font-semibold text-slate-700">รวมมอบหมาย → ส่งงาน: {minutesBetween(task.created_at, task.submitted_at)}</div></div></div></div>
              {!!task.checklist?.length && <div><h3 className="font-medium text-sm mb-2">รายการงาน</h3><div className="space-y-2">{task.checklist.sort((a, b) => a.sort_order - b.sort_order).map((item) => { const canCheck = mineTask && !busy && ['in_progress', 'revision'].includes(task.status); return <label key={item.id} title={!canCheck && mineTask && ['new', 'acknowledged'].includes(task.status) ? 'ต้องกดเริ่มทำงานก่อนจึงจะเช็กรายการได้' : undefined} className={`flex gap-3 p-2 rounded-lg bg-gray-50 ${canCheck ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'}`}><input type="checkbox" checked={item.is_completed} disabled={!canCheck} onChange={async (e) => { await toggleTaskChecklist(item.id, e.target.checked); if (me) await load(me) }} /><span className={`text-sm ${item.is_completed ? 'line-through text-gray-400' : ''}`}>{item.title}</span></label> })}</div>{mineTask && ['new', 'acknowledged'].includes(task.status) && <p className="mt-2 text-xs text-amber-600">กดเริ่มทำงานก่อน จึงจะเช็กรายการงานได้</p>}</div>}
    ...
    {submitTask && createPortal(<div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4"><p className="text-sm text-amber-700">กรุณาแนบลิงก์ผลงาน หรือกรอกข้อความส่งงานอย่างน้อย 1 รายการ</p><label className="block"><span className="block text-sm font-medium mb-2">ข้อความส่งงาน</span><textarea autoFocus rows={5} value={submitNote} onChange={(e) => { setSubmitNote(e.target.value); setError('') }} placeholder="สรุปผลงานที่ทำเสร็จ หรือปัญหาที่พบ..." className="w-full resize-none border border-gray-300 rounded-xl p-3 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" /></label><label className="block"><span className="block text-sm font-medium mb-2">ลิงก์ผลงาน</span><input type="url" inputMode="url" value={submitLink} onChange={(e) => { setSubmitLink(e.target.value); setError('') }} placeholder="https://..." className="w-full border border-gray-300 rounded-xl p-3 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" /></label><div className="grid grid-cols-2 gap-3"><button disabled={busy} onClick={() => setSubmitTask(null)} className="py-3 border rounded-xl font-semibold">ยกเลิก</button><button disabled={busy || (!submitNote.trim() && !submitLink.trim())} onClick={sendForReview} className="py-3 bg-emerald-600 text-white rounded-xl font-semibold disabled:cursor-not-allowed disabled:opacity-50">{busy ? 'กำลังบันทึก...' : submitTask.status==='review'?'บันทึกการแก้ไข':'ส่งงาน'}</button></div></div>
      </div>
    </div>, document.body)}
