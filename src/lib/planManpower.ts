...
export interface EmployeeSkill { id?:string; employee_id:string; department_name:string; process_name:string; proficiency:number; efficiency_percent:number; qualification_status:QualificationStatus; is_primary:boolean; max_concurrent_jobs:number; assessed_at?:string|null; valid_until?:string|null }
...
/** งานหลักมาก่อน แล้วจึงเทียบคะแนน Skill/ประสิทธิภาพ */
export function compareManpowerSkills(a:EmployeeSkill,b:EmployeeSkill) { return Number(b.is_primary)-Number(a.is_primary) || manpowerScore(b)-manpowerScore(a) }
export function isSupervisorResponsibility(level:ResponsibilityLevel|undefined) { return level==='supervisor'||level==='lead' }
/**
 * ผู้ปฏิบัติงานปกติมาก่อน แล้วจึงใช้หัวหน้า/ผู้เชี่ยวชาญเป็นกำลังเสริม
 * เมื่อเปิดตัวเลือกอนุญาตให้หัวหน้าทำงาน แม้กระบวนการนั้นไม่กำหนดโควตาหัวหน้า
 */
export function operatorSkillCandidates(
  skills:EmployeeSkill[],
  responsibilityOf:(employeeId:string)=>ResponsibilityLevel|undefined,
  allowSupervisorAsWorker:boolean,
) {
  const operators=skills.filter(s=>!isSupervisorResponsibility(responsibilityOf(s.employee_id))).sort(compareManpowerSkills)
  if(!allowSupervisorAsWorker)return operators
  const supervisors=skills.filter(s=>isSupervisorResponsibility(responsibilityOf(s.employee_id))).sort(compareManpowerSkills)
