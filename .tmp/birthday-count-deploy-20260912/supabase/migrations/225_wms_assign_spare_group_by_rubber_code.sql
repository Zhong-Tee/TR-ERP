-- 225: WMS assign v2 — รวมอะไหล่ตาม rubber_code ทั้งใบงาน (ไม่แยกต่อบิล)
-- เป้าหมาย:
-- - แสดงชื่อรายการอะไหล่ตาม "รหัสหน้ายาง" (pr_products.rubber_code)
-- - ถ้ารหัสเหมือนกันให้รวมเป็น 1 รายการ (sum qty) ระดับใบงาน
...
BEGIN
  ...
  -- 2) หยิบ: อะไหล่ตาม rubber_code (รวมทั้งใบงาน)
  WITH spare AS (
    SELECT
      p.rubber_code AS rc,
      sum(coalesce(oi.quantity, 1)::numeric) AS sum_q
