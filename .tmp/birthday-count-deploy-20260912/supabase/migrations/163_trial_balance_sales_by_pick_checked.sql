-- ============================================
-- 163: Trial balance sales by checked pick flow
-- - gross_sales from orders that have stock deduction (pick) in period
-- - include only orders currently in status "ตรวจสอบแล้ว"
-- - net_sales is VAT-excluded (7%) and does not subtract approved refunds
-- ============================================
...
BEGIN
  ...
  WITH picked_orders AS (
    ...
    WHERE sm.created_at >= v_month_start
