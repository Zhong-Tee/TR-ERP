...
export default function CreateAuditForm() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      showModal('error', 'สร้างใบ Audit ไม่สำเร็จ', e instanceof Error ? e.message : String(e))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-6 mt-12 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-3">
        <label className="block text-sm font-semibold text-gray-700">
          มอบหมายผู้ตรวจนับที่มีสิทธิ์ Mobile “ตรวจนับสต๊อก” <span className="text-red-500">*</span>
        </label>
        {auditors.length === 0 ? (
          <div className="text-sm text-gray-400 py-4 text-center border rounded-lg">
            ยังไม่มีผู้ใช้งานที่เปิดสิทธิ์ Mobile “ตรวจนับสต๊อก”
          </div>
        ) : (
