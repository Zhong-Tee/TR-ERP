...
export default function LeaveOverageReport() {
  ...
  const [salaryHistory, setSalaryHistory] = useState<HRSalaryHistory[]>([])
  ...
  async function load() {
    ...
    setEmployees(emps); setRequests(reqs); setTypes(leaveTypes); setOpenings(openingRows); setWorkflows((saved.data || []) as Workflow[]); setSalaryHistory((history.data || []) as HRSalaryHistory[]); setLoading(false)
  }
  useEffect(() => {
    let cancelled = false
    void (async () => {
  ...
  const rows = useMemo<ReportRow[]>(() => {
    ...
    return employees.flatMap(employee => {
      ...
      const applicableSalary = salaryHistory.find(item => item.employee_id === employee.id)
  ...
  async function setStatus(row: ReportRow, status: Workflow['status']) {
    const payload = { employee_id: row.employee.id, payroll_month: `${month}-01`, excess_days: row.excessDays, salary_base: row.salaryBase, pay_type: row.payType, deduction_amount: row.deductionAmount, status, confirmed_at: status === 'pending' ? null : new Date().toISOString(), sent_at: status === 'sent' ? new Date().toISOString() : null }
  ...
  return <div className="p-5 space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-bold text-surface-800">สรุปลาเกินสิทธิ์เพื่อหักเงินเดือน</h2><p className="text-sm text-surface-500">ยอดหัก = อัตรารายวัน × วันลาเกินสิทธิ์ โดยพนักงานประจำคิดจากฐานเงินเดือน ÷ 30 และพนักงานรายวันใช้ค่าแรงรายวัน (1 วัน = 8 ชั่วโมง)</p></div><label className="text-sm">เดือน<input type="month" value={month} onChange={e => setMonth(e.target.value)} className="ml-2 rounded-lg border px-3 py-2"/></label></div>
