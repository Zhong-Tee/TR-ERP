...
import { FiUpload, FiTrash2, FiExternalLink } from 'react-icons/fi'
import {
  upsertEmployee,
  fetchDepartments,
  fetchPositions,
...
export default function EmployeeForm({ employee, onSave }: EmployeeFormProps) {
  ...
  const [openingDocIndex, setOpeningDocIndex] = useState<number | null>(null)
  ...
  const openDocument = async (doc: DocEntry, index: number) => {
    const previewWindow = window.open('', '_blank')
    if (!previewWindow) {
      setError('เบราว์เซอร์ปิดกั้นหน้าต่างเอกสาร กรุณาอนุญาต Pop-up แล้วลองใหม่')
      return
    }

    previewWindow.opener = null
    setOpeningDocIndex(index)
    setError(null)
    try {
      const url = await getHRSignedUrl(BUCKET_DOCUMENTS, doc.url)
      previewWindow.location.href = url
    } catch (err) {
      previewWindow.close()
      setError(err instanceof Error ? `เปิดเอกสารไม่สำเร็จ: ${err.message}` : 'เปิดเอกสารไม่สำเร็จ')
    } finally {
      setOpeningDocIndex(null)
    }
  }
                ...
                documents.map((doc, i) => (
                  ...
                  >
                    <div className="min-w-0">
                      <button
                        type="button"
                        onClick={() => openDocument(doc, i)}
                        disabled={openingDocIndex === i}
                        className="flex max-w-full items-center gap-1.5 font-medium text-emerald-700 hover:text-emerald-800 hover:underline disabled:cursor-wait disabled:opacity-60"
                        title={`เปิดอ่าน ${doc.name}`}
                      >
                        <span className="truncate">{openingDocIndex === i ? 'กำลังเปิด...' : doc.name}</span>
