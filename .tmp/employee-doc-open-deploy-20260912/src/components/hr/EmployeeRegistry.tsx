...
import { isBirthdayMonth } from '../../lib/birthday'
...
export default function EmployeeRegistry() {
  ...
  const birthdayMonthCount = filtered.filter((emp) => isBirthdayMonth(emp.birth_date)).length
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-700 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span>วันเกิด</span>
                      <span
                        className="inline-flex min-w-5 items-center justify-center rounded-full bg-pink-100 px-1.5 py-0.5 text-[11px] font-semibold text-pink-700"
                        title={`เดือนเกิด ${birthdayMonthCount} คน`}
                        aria-label={`เดือนเกิด ${birthdayMonthCount} คน`}
                      >
                        {birthdayMonthCount}
                      </span>
                    </div>
                  ...
                  filtered.map((emp) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-sm text-gray-600">
                        {emp.birth_date ? (
                          <div>
                            <div>{new Date(emp.birth_date).toLocaleDateString('th-TH')}</div>
                            {isBirthdayMonth(emp.birth_date) && (
