-- Deduct stock when WMS order marked as picked/correct
...
DROP TRIGGER IF EXISTS trg_inv_deduct_wms_picked ON wms_orders;
CREATE TRIGGER trg_inv_deduct_wms_picked
  AFTER UPDATE OF status ON wms_orders
  FOR EACH ROW
  EXECUTE FUNCTION inv_deduct_stock_on_wms_picked();
