-- sales-pump / admin-pump: admin_user บนบิลมักเทียบกับ username หรืออีเมลจาก Auth (JWT)
-- ไม่ใช่แค่ us_users.email — เพิ่มการจับคู่ + ไม่สนตัวพิมพ์ + role เดิม admin-pump
...
BEGIN
  ...
  RETURN a IN (
    lower(btrim(coalesce(me.username, ''))),
    lower(btrim(coalesce(me.email, ''))),
    lower(btrim(coalesce(jwt_email, '')))
  )
  AND length(a) > 0;
END;
$$;
