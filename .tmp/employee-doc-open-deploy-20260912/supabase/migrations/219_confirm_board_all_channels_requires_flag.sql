-- คิว Confirm: PUMP เหมือนเดิมทุกบิลในสถานะ pipeline; ช่องอื่นต้อง requires_confirm_design = true
-- Backfill: ช่องที่ไม่ใช่ PUMP ให้ requires_confirm_design = false เพื่อไม่ให้บิลเก่าโผล่บอร์ดโดยไม่ตั้งใจ (ค่า default เดิมของคอลัมน์เป็น true)
-- RPC unread: ขยายเงื่อนไขเดียวกันสำหรับ sales-tr / production

UPDATE or_orders
SET requires_confirm_design = false
WHERE COALESCE(TRIM(channel_code), '') <> 'PUMP';
      ...
      AND (
        ...
        OR (
          ...
          AND (
            o.channel_code = 'PUMP'
            OR (
              COALESCE(o.requires_confirm_design, false) = true
              AND o.channel_code IS DISTINCT FROM 'PUMP'
            )
          )
        )
      );
