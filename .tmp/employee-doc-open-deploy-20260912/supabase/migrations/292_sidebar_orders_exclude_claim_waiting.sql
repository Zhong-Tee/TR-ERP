-- get_sidebar_counts.orders: ไม่รวมบิลเคลม/REQ ที่อยู่สถานะ 'รอลงข้อมูล'
-- (บิลเคลมทำงานที่แท็บบิลเคลม ไม่ใช่รอลงข้อมูลปกติ) — คงบิลเคลมในสถานะ ลงข้อมูลผิด/ตรวจสอบไม่ผ่าน ไว้เหมือนเดิม
-- อ้างอิงตัวเต็มจาก 247_sidebar_counts_warehouse_align_reorder_logic.sql เปลี่ยนเฉพาะ 3 query ของ v_orders_pending
...
BEGIN
  ...
  -- orders: ไม่รวมบิลเคลม (claim_type IS NOT NULL) ที่ยังอยู่สถานะ 'รอลงข้อมูล'
  IF v_is_sales_pump_owner THEN
    SELECT count(*) INTO v_orders_pending
    FROM or_orders
    WHERE status = ANY(v_excluded)
      AND (claim_type IS NULL OR status <> 'รอลงข้อมูล')
