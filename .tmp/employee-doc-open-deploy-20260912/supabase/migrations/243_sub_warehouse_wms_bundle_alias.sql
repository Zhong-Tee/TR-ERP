-- ============================================
-- Sub warehouse daily sheet: WMS bundle alias
-- รหัส 990000430 / 990000431 (วัตถุดิบคลังย่อย) ใช้ยอดผลิต WMS รวมจาก
-- 110000096–110000099 (ตรายาง A1–A4) แทนการจับคู่ตรงกับรหัสตัวเอง
-- ============================================
...
AS $$
  ...
  bundle_wms_open AS (
    SELECT COALESCE(SUM(o.qty), 0)::numeric AS qty
    FROM wms_orders o
  ...
  SELECT
    ...
    (
      CASE
        WHEN pr.product_code IN ('990000430', '990000431') THEN bwo.qty
        ELSE COALESCE(wo.qty, 0)
      END
    )::numeric AS wms_opening,
    (
      CASE
        WHEN pr.product_code IN ('990000430', '990000431') THEN bwd.qty
  ...
  FROM products pr
  CROSS JOIN bundle_wms_open bwo
  CROSS JOIN bundle_wms_day bwd
